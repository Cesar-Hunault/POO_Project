...
tuple<Radiographie, Patient, string> Radiologue::add_new_radio(int last_patient_id, int last_radio_id, vector<Patient> &vecpat){
    ...
    if (new_patient=="1"){
        id_patient = to_string(last_patient_id+1);
        ...
        ptr_patient= new Patient(id_patient, password, name, first_name, birth_date, gender);
    }
    if (new_patient=="0"){
    ...
    med_id = to_string(this->id_medecin);

    cout<<"Date of radiography : jj/mm/aaaa"<<endl;
    cin>>rad_date;

    num_examen = to_string(last_radio_id+1);

    Radiographie *ptr_nouvelle_radio = new Radiographie(num_examen, radio_type, ptr_patient, med_id, rad_date, state);
    ...
    return make_tuple(return_radio, return_patient, new_patient);
                ...
                case 2:
                    { 
                        ...
                        for(int i=1; i<=nbr_radio; i++){
                            cout<<"Liez une radiographie :"<<endl;
                            getline (cin, img);//permet de récuperer les espaces
                            cout<<"Quel est son numéro de prise :"<<endl;
                        ...
                        ptr_med_res->medecin_result_display();
                        cout<<"##################"<<endl;
                        ptr_pat_res->patient_result_display();
